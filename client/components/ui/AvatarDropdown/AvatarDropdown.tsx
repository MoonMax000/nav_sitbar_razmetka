          ...
          {menuItems.map((item) => {
            const content = (
              <div className='flex items-center gap-3 px-4 py-3 hover:bg-[#181B20] transition-colors duration-200 cursor-pointer group'>
                <div className='text-webGray group-hover:text-primary transition-colors duration-200'>{item.icon}</div>
                <span className='text-white text-[15px] font-semibold group-hover:text-primary transition-colors duration-200'>{item.label}</span>
              </div>
            );

            if (item.to) {
              return (
                <Link key={item.id} to={item.to} onClick={() => setIsOpen(false)}>
                  {content}
                </Link>
              );
            }
