...
import { getSocialPostById, SocialPost } from "@/data/socialPosts";
  ...
  const renderArticle = () => (
    <div className="mx-auto flex w-full max-w-[680px] flex-col gap-6 rounded-3xl border border-[#181B22] bg-black/70 p-6 text-white shadow-[0_20px_60px_-40px_rgba(0,0,0,0.9)]">
      ...
      <div className="space-y-4">
        ...
        {post.body && (
          <p className="whitespace-pre-line text-[15px] leading-relaxed text-[#E7E9EA]">{post.body}</p>
        )}
        {hashtags.length > 0 && (
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 text-sm text-[#8B98A5]">
        ...
        {typeof post.views === "number" && <div>{post.views} Views</div>}
      </div>
      ...
      <div className="rounded-2xl border border-[#181B22] bg-black/60 p-4">
